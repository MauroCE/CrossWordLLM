...
import torch
import pickle
from reversing_model import MultiTokenGPT, estimate_loss, get_batch
from config import MultiTokenConfig
...
if __name__ == "__main__":
    ...
    # Save model
    torch.save(model.state_dict(), "models/multi_token_{}_{}_new.pth".format(eval_interval, max_iters))
    with open("losses/multi_token_training_{}_{}_new.pkl".format(eval_interval, max_iters), "wb") as file:
        pickle.dump(training_losses, file)
    with open("losses/multi_token_validation_{}_{}_new.pkl".format(eval_interval, max_iters), "wb") as file:
        pickle.dump(validation_losses, file)

    # Save final time
    total_time = time.time() - start_time
    print("Total time: ", total_time)
    with open("timings/multi_token_{}_{}_new.pkl".format(eval_interval, max_iters), "wb") as file:
